...
private:
    ...
    std::vector<VkFence> inFightFences;
...
private:
    ...
    void createSyncObjects()
    {
        ...
        inFightFences.resize(MAX_FRAMES_IN_FLIGHT);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS
                ...
                || vkCreateFence(device, &fenceInfo, nullptr, &inFightFences[i]) != VK_SUCCESS)
            {
    ...
    void drawFrame()
    {
        vkWaitForFences(device, 1, &inFightFences[currentFrame], VK_TRUE, UINT64_MAX);
        vkResetFences(device, 1, &inFightFences[currentFrame]);
        ...
        if (vkQueueSubmit(graphicQueue, 1, &submitInfo, inFightFences[currentFrame])) {
    ...
    void cleanup()
    {
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            vkDestroyFence(device, inFightFences[i], nullptr);
        }
    ...
    QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device)
    {
        ...
        std::cout << std::boolalpha << graphicFamily.has_value() << std::endl;
    ...
    VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availibleFormats)
    {
        ...
        {
            if (availibleFormat.format == VK_FORMAT_B8G8R8A8_SRGB && availibleFormat.colorSpace == VK_COLORSPACE_SRGB_NONLINEAR_KHR)
